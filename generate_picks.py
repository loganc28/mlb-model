...
def build_html(data):
    ...
    css = (
        ...
        ':root{'
        '--bg:#0A0C0F;--surface:#111418;--surface2:#181C22;--surface3:#1E2330;'
        '--border:#1A1F2A;--border2:#222836;'
        '--text:#F0F4F8;--muted:#5C6880;--faint:#1E2330;--subtle:#8A95A8;'
        '--gold:#F5C842;--gold2:#FFD966;--green:#22D47A;--red:#F04D5A;--blue:#4B9EF5;--purple:#A78BFA;'
        '--radius:14px;--radius-sm:8px;--radius-xs:6px;'
        '--tier-max:#F5C842;--tier-a:#22D47A;--tier-b:#4B9EF5;--tier-c:#8A95A8;'
        '}'
        'body{font-family:"Inter",sans-serif;background:var(--bg);color:var(--text);'
        'padding:0;max-width:800px;margin:0 auto;min-height:100vh;font-size:15px;'
        'background-image:radial-gradient(ellipse at 50% 0%,#0D1520 0%,#0A0C0F 60%)}'
        '.page-header{padding:2.5rem 1.5rem 1.5rem;border-bottom:1px solid var(--border)}'
        '.brand{font-size:10px;font-weight:600;letter-spacing:.22em;text-transform:uppercase;'
        'color:var(--gold);margin-bottom:8px;opacity:.7}'
        '.page-title{font-size:36px;font-weight:900;letter-spacing:-.04em;'
        'color:var(--text);margin-bottom:4px;line-height:1}'
        '.page-subtitle{font-size:13px;color:var(--muted);display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-top:8px}'
        '.page-subtitle a{color:var(--subtle);text-decoration:none;font-weight:500}'
        '.page-subtitle a:hover{color:var(--text)}'
        '.divider{color:var(--border2)}'
        '.stats-bar{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;padding:1.25rem 1.5rem;border-bottom:1px solid var(--border)}'
        '.stat-card{background:var(--surface);border:1px solid var(--border);'
        'border-radius:var(--radius);padding:14px 16px}'
        '.stat-val{font-size:28px;font-weight:900;letter-spacing:-.03em;line-height:1}'
        '.stat-lbl{font-size:9px;color:var(--muted);margin-top:6px;text-transform:uppercase;'
        'letter-spacing:.12em;font-weight:600}'
        '.section-label{font-size:9px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;'
        'color:var(--muted);padding:1.5rem 1.5rem 0.75rem;display:flex;align-items:center;gap:12px}'
        '.section-label::after{content:"";flex:1;height:1px;background:var(--border)}'
        '.picks-list{padding:0 1rem 2rem}'
        '.pick-card{background:var(--surface);border:1px solid var(--border);'
        'border-radius:var(--radius);margin-bottom:8px;overflow:hidden;position:relative}'
        '.pick-card::before{content:"";position:absolute;left:0;top:0;bottom:0;width:3px;border-radius:3px 0 0 3px}'
        '.pick-card.tier-MAX::before{background:var(--tier-max)}'
        '.pick-card.tier-A::before{background:var(--tier-a)}'
        '.pick-card.tier-B::before{background:var(--tier-b)}'
        '.pick-card.tier-C::before{background:var(--tier-c)}'
        '.pick-card.tier-MAX{border-color:#F5C84220;background:linear-gradient(135deg,#111418 0%,#141209 100%)}'
        '.pick-card.tier-A{border-color:#22D47A18}'
        '.pick-card.tier-B{border-color:#4B9EF515}'
        '.pick-card.tier-SKIP{opacity:.35}'
        '.pick-card.tier-WATCH{opacity:.6}'
        '.card-inner{padding:1rem 1.1rem 1rem 1.4rem}'
        ...
        '.tier-badge{display:inline-flex;align-items:center;font-size:9px;font-weight:700;'
        'letter-spacing:.14em;text-transform:uppercase;padding:3px 9px;'
        'border-radius:20px;font-family:"JetBrains Mono",monospace}'
        '.tier-badge.MAX{background:#F5C84218;color:var(--gold);border:1px solid #F5C84230}'
        '.tier-badge.A{background:#22D47A12;color:var(--green);border:1px solid #22D47A28}'
        '.tier-badge.B{background:#4B9EF510;color:var(--blue);border:1px solid #4B9EF525}'
        '.tier-badge.C{background:#8A95A810;color:var(--subtle);border:1px solid #8A95A820}'
        '.tier-badge.WATCH{background:#8A95A808;color:var(--muted);border:1px solid #8A95A815}'
        '.tier-badge.SKIP{background:#1E232018;color:var(--muted);border:1px solid #1E232025}'
        '.units-badge{font-family:"JetBrains Mono",monospace;font-weight:700;font-size:14px;color:var(--gold)}'
        '.odds-badge{font-family:"JetBrains Mono",monospace;font-weight:600;font-size:14px;'
        'color:var(--text);background:var(--surface2);padding:3px 10px;border-radius:var(--radius-xs);'
        'border:1px solid var(--border2)}'
        '.pick-name{font-size:26px;font-weight:900;letter-spacing:-.03em;margin-bottom:3px;line-height:1.05;color:var(--text)}'
        '.pick-sub{font-size:11px;color:var(--muted);margin-bottom:10px;'
        'display:flex;gap:6px;align-items:center;flex-wrap:wrap}'
        '.game-label{color:var(--subtle)}'
        '.game-time{font-family:"JetBrains Mono",monospace;color:var(--muted)}'
        '.score-pill{font-family:"JetBrains Mono",monospace;font-size:10px;font-weight:600;'
        'background:var(--surface3);color:var(--muted);padding:2px 8px;border-radius:var(--radius-xs)}'
        '.score-pill.live{background:#F5C84212;color:var(--gold);border:1px solid #F5C84225}'
        '.score-pill.final{background:#22D47A10;color:var(--green)}'
        '.sp-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px}'
        '.sp-box{background:var(--surface2);border:1px solid var(--border);'
        'border-radius:var(--radius-sm);padding:8px 12px}'
        '.sp-lbl{font-size:8px;color:var(--muted);text-transform:uppercase;'
        'letter-spacing:.12em;margin-bottom:4px;font-weight:600}'
        '.sp-name{font-size:13px;font-weight:600;color:var(--text)}'
        '.ev-strip{display:flex;align-items:center;justify-content:space-between;margin-bottom:5px}'
        '.ev-nums{display:flex;align-items:center;gap:8px;font-size:11px;font-family:"JetBrains Mono",monospace}'
        '.win-pct{color:var(--text);font-weight:600}'
        '.ev-sep{color:var(--border2)}'
        '.impl-pct{color:var(--muted)}'
        '.ev-badge{font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;'
        'font-family:"JetBrains Mono",monospace}'
        '.ev-badge.MAX,.ev-badge.A{background:#22D47A12;color:var(--green);border:1px solid #22D47A25}'
        '.ev-badge.B{background:#4B9EF510;color:var(--blue);border:1px solid #4B9EF525}'
        '.ev-badge.C{background:#8A95A810;color:var(--subtle);border:1px solid #8A95A820}'
        '.ev-bar{height:2px;background:var(--surface3);border-radius:2px;margin-bottom:10px}'
        '.ev-fill{height:100%;border-radius:2px}'
        '.ev-fill.MAX{background:linear-gradient(90deg,var(--gold),var(--gold2))}'
        '.ev-fill.A{background:var(--green)}'
        '.ev-fill.B{background:var(--blue)}'
        '.ev-fill.C{background:var(--subtle)}'
        '.key-edge{background:var(--surface2);border-left:2px solid var(--gold);padding:9px 12px;'
        'margin-bottom:10px;font-size:12px;font-weight:500;color:var(--text);line-height:1.55;'
        'border-radius:0 var(--radius-sm) var(--radius-sm) 0}'
        '.details{border-top:1px solid var(--border);padding-top:8px;margin-top:2px}'
        '.detail-row{display:flex;gap:10px;padding:4px 0;font-size:12px;line-height:1.5}'
        '.detail-lbl{font-weight:600;color:var(--muted);flex-shrink:0;width:72px;font-size:10px;'
        'text-transform:uppercase;letter-spacing:.08em;padding-top:1px}'
        '.detail-val{color:var(--subtle);flex:1;line-height:1.5}'
        '.flag{font-size:11px;background:#F5C84208;color:#F5C84290;padding:5px 10px;'
        'border-radius:var(--radius-sm);margin-bottom:8px;border:1px solid #F5C84215;line-height:1.5}'
        '.watch-reason{font-size:11px;color:var(--muted);margin-top:6px;'
        'padding-top:8px;border-top:1px solid var(--border);font-style:italic;line-height:1.5}'
        '.skip-reason{font-size:11px;color:var(--muted);margin-top:6px;line-height:1.5}'
        'footer{font-size:11px;color:var(--muted);margin-top:2rem;text-align:center;'
        'padding:1.5rem 1.5rem 3rem;border-top:1px solid var(--border);line-height:2;opacity:.6}'
        '@media(max-width:500px){'
        '.stats-bar{grid-template-columns:repeat(2,1fr)}'
        '.sp-grid{grid-template-columns:1fr}'
        '.page-title{font-size:28px}'
        '.pick-name{font-size:22px}'
        '.picks-list{padding:0 0.5rem 2rem}'
        '}'
        '</style>'
    )
